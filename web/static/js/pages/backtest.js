...
async function loadBacktestPage() {
  ...
  renderLineChart("bt-roi-chart", "Cumulative ROI", data.roiTrend || [], "rgba(13, 110, 253, 1)");

  const labels = (hitTrend.hit2 || []).map((row) => row.label);
  renderMultiLineChart("bt-hit-chart", labels, [
    { label: "Hit 2+", data: (hitTrend.hit2 || []).map((row) => row.value), borderColor: "rgba(25, 135, 84, 1)", backgroundColor: "rgba(25, 135, 84, 0.08)", tension: 0.25 },
    { label: "Hit 3+", data: (hitTrend.hit3 || []).map((row) => row.value), borderColor: "rgba(255, 193, 7, 1)", backgroundColor: "rgba(255, 193, 7, 0.08)", tension: 0.25 },
    { label: "Hit 4+", data: (hitTrend.hit4 || []).map((row) => row.value), borderColor: "rgba(220, 53, 69, 1)", backgroundColor: "rgba(220, 53, 69, 0.08)", tension: 0.25 },
  ]);
